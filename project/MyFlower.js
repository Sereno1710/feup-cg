...
export class MyFlower extends CGFobject {
  constructor(scene, x, y, z) {
    ...
    this.initMaterials();
    this.petalMaterials = [this.petalMaterial1, this.petalMaterial2, this.petalMaterial3];
    this.petalMaterial = this.petalMaterials[Math.floor(Math.random() * 3)];
    this.randomize();
    this.coordinates = {x: x, y: y, z: z};
    this.stem = new MyStem(scene, this.stem_radius, this.stem_size,this.incline, this.petal_number);
    this.receptacle = new MyReceptacle(scene, this.receptacle_radius, this.stem_size, this.stem_radius, Math.PI/4, this.petal_number);
    ...
    this.petals = new MyPetals(scene, petal_size, Math.PI/4, this.stem_size, this.petal_number, this.receptacle_radius, this.petal_random);
  ...
  initMaterials(){
      ...
      this.petalMaterial1.setTexture(new CGFtexture(this.scene, "images/petaltexture1.png"));
      ...
      this.petalMaterial2.setTexture(new CGFtexture(this.scene, "images/petaltexture2.png"));
      ...
      this.petalMaterial3.setTexture(new CGFtexture(this.scene, "images/petaltexture3.png"));
      ...
      this.receptacleMaterial.setTextureWrap('MIRRORED_REPEAT', 'MIRRORED_REPEAT');
  ...
  randomize(){
    ...
    for(var i = 0; i < this.stem_size-1; i++){
      stem_angles.push( Math.random()*0.2 * (Math.random() > 0.5 ? 1 : -1)* (Math.random() > 0.2 ? 1 : 0));
    }
