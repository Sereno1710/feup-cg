  ...
  constructor(scene, x, y, z, cols, rows) {
    ...
    for (let i = 0; i < this.rows; i++) {
      for (let j = 0; j < this.cols; j++) {
        flowers.push(new MyFlower(this.scene, new MyPollen(this.scene)));
      }
    }
    ...
    for (let i = 0; i < this.rows; i++) {
      for (let j = 0; j < this.cols; j++) {
        ...
        this.scene.translate(
          (this.coordinates.x + i) * 10,
          this.coordinates.y,
          (this.coordinates.z + j) * 10
        );
  ...
  updateGarden(rows, cols) {
    ...
    if (this.rows * this.cols < this.flowers.length) {
      ...
      for (let i = this.flowers.length; i < this.rows * this.cols; i++) {
        this.flowers.push(new MyFlower(this.scene, new MyPollen(this.scene)));
      }
    }
    this.display();
  }
